...
import { BufferedString, NonBufferedString, StringBuilder } from "./bufferedString";
import { JsonPrimitive } from "@streamparser/json/dist/mjs/utils/types/jsonTypes";
...
export class Tokenizer {
        ...
        switch (this.state) {
          case TokenizerStates.START:
            ...
            if (this.separatorBytes && n === this.separatorBytes[0]) {
              if (this.separatorBytes.length === 1) {
                ...
                this.onToken(TokenType.SEPARATOR, this.separator as string);
                continue;
              }
          ...
          case TokenizerStates.STRING_DEFAULT:
            if (n === charset.QUOTATION_MARK) {
              ...
              this.state = TokenizerStates.START;
              this.onToken(TokenType.STRING, string);
              continue;
            }
  ...
  private emitNumber(): void {
    this.onToken(TokenType.NUMBER, this.parseNumber(this.bufferedNumber.toString()));
  }
  ...
  public onToken(token: TokenType, value?: JsonPrimitive): void {
